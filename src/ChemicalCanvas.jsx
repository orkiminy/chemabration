...
import correctStructure from "./data/correctStructure.json"; // <-- your answer file
...
export default function ChemicalEditor() {
  ...
  const createHexagon = () => {
    ...
    const center = gridPoints.reduce((best, p) => {
      const d = Math.hypot(p.x - WIDTH / 2, p.y - HEIGHT / 2);
      return d < best.dist ? { point: p, dist: d } : best;
    }, { point: gridPoints[0], dist: Infinity }).point;

    const dx = GRID_SPACING * 2;
    const dy = GRID_SPACING * 2 * Math.sin(Math.PI / 3);
    ...
    const newAtoms = hexPoints.map((p, i) => ({
      id: Date.now() + i,
      x: p.x,
      y: p.y,
      label: "C",
  ...
  /* ---------- CHECK ANSWER ---------- */
  const checkAnswer = () => {
    // Simple comparison: number of atoms and labels + bonds length and style
  ...
  return (
    <div style={{ fontFamily: "Arial" }}>
      ...
      <div style={{
        ...
        <button onClick={createHexagon}>Hexagon</button>
